...
def _invalidate_setups_hit_tp_sl(
    ...
    *,
    ...
    invalidation_sort_cols = [c for c in ("timestamp", "model", "side", "entry") if c in df_e.columns]
...
# normalize
    ...
    # ------------------------------------------------------------
    # PHASE_GUARD enable flag (OPT-IN)
    # Default OFF in KPI_VALIDATION even if --phase_guard passed
    # ------------------------------------------------------------
    try:
        _rm = (os.environ.get("RUN_MODE") or "KPI_VALIDATION").strip().upper()
        if _rm not in ("KPI_VALIDATION", "DEV_DEBUG"):
            _rm = "KPI_VALIDATION"
        phase_guard_enabled = bool(phase_guard) and (_rm == "DEV_DEBUG")
    except Exception:
        phase_guard_enabled = False
    # ============================================================
    # Normalize ctx phase -> scalar (avoid pandas Series issues)
    # MUST be before PHASE_GUARD and before generate_entries_from_ctx
    # ============================================================
    ...
    if phase_scalar not in {"PHASE_TREND_UP", "PHASE_TREND_DOWN", "PHASE_RANGE"}:
        phase_scalar = "PHASE_RANGE"
    # ------------------------------------------------------------
    # SAFETY (OPT-IN): phase/trend mismatch guard
    # PHASE_TREND_UP + trend_dir=DOWN -> override to PHASE_RANGE (only if enabled)
    # ------------------------------------------------------------
    try:
        tdir = None
        ...
        # use scalar (avoid Series issues)
        ph = str(phase_scalar or "PHASE_RANGE").upper()

        if (ph == "PHASE_TREND_UP") and (tdir == "DOWN"):
            if phase_guard_enabled:
                # override (behavior change) - DEV only / opt-in
                ctx["phase"] = "PHASE_RANGE"
                phase_scalar = "PHASE_RANGE"
                phase_authority_source = "phase_guard_override"
                if debug_regime:
                    print(
                        f"[{now_utc_str()}] [PHASE_GUARD][{bybit_symbol}] override PHASE_RANGE (phase={ph}, trend_dir={tdir})")
            else:
                phase_authority_source = "phase_guard_no_override"
                # KPI_VALIDATION-safe: log only, no override
                if debug_regime or debug_entry_filters:
                    print(
                        f"[{now_utc_str()}] [PHASE_GUARD][{bybit_symbol}] mismatch phase={ph} trend_dir={tdir} (no override)")
    except Exception:
        pass
