...
def main():
    ...
    # Align with live_signal_runner behavior: generate setups on the latest window.
    # Using the full history can yield 0 entries due to cooldown / regime gating differences.
    p.add_argument(
        "--emit_last_candles",
        type=int,
        default=200,
        help="Use only the latest N candles for entry generation (default: 200)",
    )
    ...
    for sym in symbols:
        ...
        # Mirror live runner (default 200 last candles)
        if int(args.emit_last_candles) > 0 and len(ctx) > int(args.emit_last_candles):
            ctx = ctx.tail(int(args.emit_last_candles)).reset_index(drop=True)
