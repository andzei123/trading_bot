...
def run_paper_adapter(
    *,
    ...
    """
    Thin adapter: bridge finalized signals into paper_executor.
    No business logic, no filtering, no mutation beyond existing contract.
    """
    try:
        diag_log(
        ...
        if "signal_ts" not in df_sig.columns:
            ...
            df_sig["signal_ts"] = pd.to_datetime(
                df_sig["signal_ts"], utc=True, errors="coerce"
            ).fillna(pd.to_datetime(latest_ts, utc=True, errors="coerce"))
                ...
                if _paper_trades_path.exists() and _paper_trades_path.stat().st_size > 0:
                    ...
                    if len(_df_after) > _paper_before_n:
                        ...
                        if "status" in _df_new.columns:
                            _df_new_open = _df_new[
                                _df_new["status"].astype(str).str.upper() == "OPEN"
                            ]
