...
class PortfolioState:
    ...
    def save(self) -> None:
        ...
        # Stable serialization => stable hash/mtime if unchanged
        new_txt = json.dumps(data, indent=2, sort_keys=True)

        # Dirty-save: avoid rewriting identical content
        if self.path.exists():
            try:
                old_txt = self.path.read_text(encoding="utf-8")
                if old_txt == new_txt:
                    return
            except Exception:
                pass

        self.path.write_text(new_txt, encoding="utf-8")
