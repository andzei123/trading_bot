...
def v1(seq=2):
    d={k:"" for k in R1_V1_FIELDS}; d.update(schema_version=R1_V1_SCHEMA_VERSION,sequence=seq,operation_identity="o",canonical_setup_key="k",event_type="AUTHORITY_BLOCKED",recorded_at_utc="2026-09-08T18:00:00Z",environment="TESTNET",origin="https://api-testnet.bybit.com",account_identity="a",client_order_id="c",client_identity_digest="d",request_fingerprint="f",request_fingerprint_version="v",reservation_id="r",attempt_id="a",symbol="BTCUSDT",operation="CREATE",exchange_order_id="",proof_bundle_digest="p",classification="BLOCKED",block_mutations=False,query_required=False,exchange_ret_code="",exchange_ret_message="",reason="x"); return d
def test_mixed_global_sequence():
    raw=(json.dumps(v0())+'\n'+json.dumps(v1())+'\n').encode(); u,l,r=_parse_union_records(raw); assert len(u)==2 and len(l)==1 and len(r)==1
