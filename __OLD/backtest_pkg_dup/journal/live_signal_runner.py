...
def now_utc_str() -> str:
    ...
    # pandas version differences: utcnow() may be tz-aware already
    if ts.tzinfo is None:
        ts = ts.tz_localize("UTC")
    else:
        ts = ts.tz_convert("UTC")
...
def _ensure_live_entries_csv(path: Path) -> None:
    """Create empty live_entries.csv with header if missing."""
    try:
        _ensure_parent(path)
        if not path.exists():
            pd.DataFrame(columns=LIVE_ENTRIES_COLUMNS).to_csv(path, index=False)
    except Exception:
        # Never crash the runner because of a diagnostic output file.
        pass
...
def run_once(
    ...
    entries = generate_entries_from_ctx(
        ctx,
        enable_trend=getattr(decision, 'enable_trend', True),
        enable_range_short=getattr(decision, 'enable_range_short', True),
        enable_range_long=getattr(decision, 'enable_range_long', False),
rr=rr,
    ...
    # ------------------------------------------------------------
    # Backfill / historical test helper: keep signals from last N candles
    # NOTE: this is for *testing* signal flow on recent history.
    # In true live mode you normally want only the latest candle.
    # ------------------------------------------------------------
    if emit_last_candles and int(emit_last_candles) > 0 and "timestamp" in df_e.columns:
        try:
            latest_candle_ts = pd.to_datetime(candles["timestamp"].iloc[-1], utc=True)
            cutoff = latest_candle_ts - pd.Timedelta(minutes=int(bybit_interval) * int(emit_last_candles))
            before = len(df_e)
            ...
            if debug_regime:
                print(f"[{now_utc_str()}] [DEBUG] emit_last_candles={emit_last_candles} cutoff={cutoff} kept={len(df_e)}")
            if df_e.empty:
                if debug_regime:
    ...
    if not disable_portfolio:
        try:
            ...
            if emit_last_candles and int(emit_last_candles) > 1:
            ...
            backfill_mode = bool(emit_last_candles and int(emit_last_candles) > 1)
            if backfill_mode:
...
def _state_path_for_symbol(base_state_path: str, source: str, symbol: str, interval: str) -> Path:
    """Per-symbol+interval last_seen state (prevents cross-symbol blocking)."""
    base = Path(base_state_path)
    sym = str(symbol).strip().upper()
    itv = str(interval).strip()
    base_norm = str(base).replace("\\", "/")

    # default/legacy -> keep under exports_live/state
    if base_norm.endswith("backtest/journal/live_state.txt") or base.name == "live_state.txt":
        out_dir = Path("backtest/journal/exports_live/state")
        out_dir.mkdir(parents=True, exist_ok=True)
        return out_dir / f"{source}_{sym}_{itv}_last_seen.txt"

    # custom path -> suffix in same dir
    out_dir = base.parent if str(base.parent) not in ("", ".") else Path(".")
    out_dir.mkdir(parents=True, exist_ok=True)
    suffix = base.suffix or ".txt"
    return out_dir / f"{base.stem}_{source}_{sym}_{itv}{suffix}"
    ...
    if str(getattr(args, "risk_guard_csv", "")).strip():
        try:
            if str(getattr(args, "risk_guard_month", "auto")).lower() == "auto":
                month = _utc_month_str(pd.Timestamp.now(tz="UTC"))
            else:
