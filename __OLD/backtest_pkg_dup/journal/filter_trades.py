...
def label_tts_tdp(c: pd.DataFrame, RANGE_LEN: int | None = None) -> pd.DataFrame:
    ...
    # Debug is OFF by default to keep live console clean.
    # Enable with env:  set TTS_DEBUG=1
    TTS_DEBUG = str(os.getenv("TTS_DEBUG", "0")).strip().lower() in ("1", "true", "yes", "y")
    ...
    if TTS_DEBUG:
        ...
        for _col in ["impulse_atr", "range_width_atr", "breakout_up_atr"]:
            _s = pd.to_numeric(c[_col], errors="coerce").dropna()
            if len(_s):
                print(f"{_col} q:", _s.quantile([0.5, 0.7, 0.8, 0.9]).to_dict())
...
def add_phase(ctx: pd.DataFrame) -> pd.DataFrame:
    """
    Adds ctx['phase'] based on regime + trend_dir.

    Robust behavior (important for LIVE):
      - If trend_dir is missing/empty, we fall back to htf_trend.
      - If regime is missing/empty, we still derive PHASE_TREND_* from trend.
    ...
    regime = out.get("regime", pd.Series(index=out.index, dtype="object"))
    tdir = out.get("trend_dir", pd.Series(index=out.index, dtype="object"))
    htf = out.get("htf_trend", pd.Series(index=out.index, dtype="object"))

    # normalize
    regime_u = regime.astype(str).str.upper().replace("NAN", "").fillna("")
    tdir_u = tdir.astype(str).str.upper().replace("NAN", "").fillna("")
    htf_u = htf.astype(str).str.upper().replace("NAN", "").fillna("")

    # fallback: if trend_dir empty -> use htf_trend
    tdir_eff = tdir_u.where(tdir_u.ne(""), htf_u)
    ...
    phase = np.where(
        ...
        np.where(
            tdir_eff == "UP",
            "PHASE_TREND_UP",
            np.where(tdir_eff == "DOWN", "PHASE_TREND_DOWN", "PHASE_UNKNOWN"),
        ),
    )

    out["trend_dir"] = tdir_eff
    out["phase"] = phase
    return out
